  ...
  return (
    <div className="flex flex-col bg-white border border-gray-100 shadow-sm group">
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex items-center gap-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-4">
        ...
        <h3 className="text-xl font-black text-[#001f3f] mb-3 group-hover:text-[#002855] transition-colors leading-tight">
          <Link href={`/blog/${slug}`}>{title}</Link>
        </h3>
        <p className="text-gray-500 text-sm mb-6 flex-1 line-clamp-3">{excerpt}</p>
        <Link href={`/blog/${slug}`} className="text-[#001f3f] font-bold text-xs uppercase tracking-widest hover:text-[#facc15] transition-colors inline-flex items-center">
          Read More →
        </Link>
      </div>
    </div>
