    ...
    <PageLayout>
      ...
      <section className="bg-[#001f3f] text-white py-16 md:py-20 relative overflow-hidden text-center md:text-left">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <h1 className="text-3xl md:text-5xl font-semibold mb-4 leading-tight">Video Gallery</h1>
          <div className="h-1 w-20 bg-[#facc15] mb-8 mx-auto md:mx-0"></div>
          <p className="text-base md:text-base text-white/70 max-w-2xl font-medium mx-auto md:mx-0">Watch the incredible stories of transformation and healing through our global missions.</p>
        </div>
      </section>
      ...
      <SectionWrapper className="bg-gray-50">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {videos.map((video, index) => (
            <div key={index} className="group relative bg-white overflow-hidden shadow-lg cursor-pointer flex flex-col h-full">
              <div className="relative aspect-video overflow-hidden">
                ...
                {/* Duration Badge */}
                <div className="absolute bottom-3 right-3 md:bottom-4 md:right-4 bg-black/80 text-white text-[8px] md:text-[10px] font-semibold px-2 py-1">
                  {video.duration}
                </div>
              </div>
              
              <div className="p-6 md:p-8 border-b-4 border-transparent group-hover:border-[#facc15] transition-colors transition-transform transition-shadow flex-1">
                ...
                <h3 className="text-[#001f3f] text-xl md:text-2xl font-semibold leading-tight group-hover:text-[#facc15] transition-colors line-clamp-2">{video.title}</h3>
              </div>
            </div>
          ))}
        </div>
